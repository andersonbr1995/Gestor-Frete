...
st.markdown('<div class="sub-header">Controle com Custo x 3 automático, múltiplos complementos e simulador inteligente de cargas.</div>', unsafe_allow_html=True)

# Sidebar - Dados da Expert e Margem
...
with tab1:
    ...
    if executar_calculo:
        ...
        with m3:
            st.metric("Receita por Km", f"R$ {receita_por_km:.2f} / km")
        with m4:
        ...
        with col_m2:
            ...
            if quanto_falta_vezes_tres > 0:
                st.warning(f"⚠️ Para atingir a meta de **Custo x 3** (R$ {faturamento_ideal_calc:,.2f}), falta conseguir mais **R$ {quanto_falta_vezes_tres:,.2f}**.")
            else:
